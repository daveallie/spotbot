  ...
  robot.hear spotify.link, (msg) ->
    msg.http(spotify.uri msg.match[0]).get() (err, res, body) ->
      if res.statusCode is 200
        ...
        if (bitlyAccessToken?)
          msg.http("https://api-ssl.bitly.com").path("/v3/shorten?access_token=#{bitlyAccessToken}&longUrl=http%3A%2F%2Fs.daveallie.com%2F%3Furi%3D#{spoturi}").header('Accept', 'application/json').get() (err, res, body) ->
            if res.statusCode is 200
              data_temp = JSON.parse(body)
              msg.send mess+" [#{data_temp.data.url}]"
        else
          msg.send mess+" [http://s.daveallie.com/?uri=#{spoturi}]"
